...
axi_soc_versal_core=$(dirname $(readlink -f $0))
aes_stream_drivers=$(realpath $axi_soc_versal_core/../aes-stream-drivers)
...
# Check if the Yocto versal-user.conf file exists
if [ ! -f "$hwDir/Yocto/versal-user.conf" ]; then
   echo "File $hwDir/Yocto/versal-user.conf does NOT exist"
   exit 1
fi
...
echo "MACHINE_ESSENTIAL_EXTRA_RRECOMMENDS:append = \" axistreamdma aximemorymap\"" >> $proj_dir/sources/meta-user/conf/machine/versal-user.conf
echo "MACHINE_ESSENTIAL_EXTRA_RRECOMMENDS:append = \" axistreamdma aximemorymap\"" >> $proj_dir/sources/meta-user/conf/machine/versal-user.conf
...
# xrt is required for running xmutil executable
echo "IMAGE_INSTALL:append = \" xrt\"" >> $proj_dir/build/conf/local.conf
...
if [ ! -f "$proj_dir/build/tmp/deploy/images/versal-user/boot.bin" ]; then
...
# Default file list
fileList="linux/system.pdi linux/BOOT.BIN linux/boot.scr linux/image.ub"
